...
const BCRP_LEGACY_CATEGORY_NAMES = new Set([
  'KCRP | Welcome Center',
  'KCRP | Community',
  'KCRP | Operations',
  'KCRP | Staff Area',
  ...
  'BCRP | Tickets',
  'BCRP | Voice',
  'BCRP | Under Development',
  'King County Roleplay',
  'KCRP',
  'Old Setup',
  'Legacy Setup'
]);

const BCRP_LEGACY_CHANNEL_NAMES = new Set([
  'welcome', 'dashboard', 'assistance', 'verify',
  'announcements', 'sessions', 'polls', 'general', 'media', 'suggestions',
  'departments', 'applications', 'application-results', 'application-review',
  'staff-announcements', 'staff-chat', 'staff-logs', 'moderation-logs', 'staff-training',
  'civilian-lounge', 'leo-lounge', 'fire-ems-lounge', 'staff-lounge',
  'support', 'ticket-panel', 'ticketpanel', 'verification', 'rules', 'ingame-rules',
  'discord-regulations', 'session-log', 'notice-board', 'development'
]);
...
async function cleanupLegacyBcrp(guild) {
  ...
  // /cleanup is intentionally different from /setup rebuild:
  // it removes the CURRENT /setup-created BCRP structure too.
  // Unrelated server categories/channels are left alone.
  const setupCategoryNames = new Set([
    ...Object.values(BCRP_SETUP_CATEGORIES),
    ...BCRP_LEGACY_CATEGORY_NAMES
  ]);

  // Delete every channel inside a BCRP setup category first, including
  // dynamically-created ticket channels.
  for (const category of [...guild.channels.cache.values()]) {
    if (category.type !== ChannelType.GuildCategory) continue;
    if (!setupCategoryNames.has(category.name)) continue;
    ...
    for (const channel of children) {
      await channel
        .delete('BlancoCountyRP /cleanup removing /setup channels')
        .then(() => deletedChannels.push(channel.name))
        .catch(() => {});
    }
  }

  // Delete standalone legacy/setup channels that are not inside a setup category.
  for (const channel of [...guild.channels.cache.values()]) {
    if (!channel || channel.type === ChannelType.GuildCategory) continue;
    if (!BCRP_LEGACY_CHANNEL_NAMES.has(channel.name)) continue;

    await channel
      .delete('BlancoCountyRP /cleanup removing legacy /setup channel')
      .then(() => deletedChannels.push(channel.name))
      .catch(() => {});
  }

  // Delete all current and legacy BCRP setup categories.
  for (const category of [...guild.channels.cache.values()]) {
    if (category.type !== ChannelType.GuildCategory) continue;
    if (!setupCategoryNames.has(category.name)) continue;

    await category
      .delete('BlancoCountyRP /cleanup removing /setup category')
      .then(() => deletedCategories.push(category.name))
      .catch(() => {});
  }
  ...
  const legacyRoles = new Set([
    ...
    'Blanco County EMS',
    'Blanco County Fire & EMS',
    'Blanco County EMS & Fire Department'
  ]);

  for (const role of [...guild.roles.cache.values()]) {
    if (!role.managed && legacyRoles.has(role.name)) {
      await role
        .delete('BlancoCountyRP /cleanup legacy department role')
        .then(() => deletedRoles.push(role.name))
        .catch(() => {});
    }
  }
