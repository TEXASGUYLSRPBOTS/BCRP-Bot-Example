...
async function setupBcrpServer(guild, { rebuild = true, visibility = 'recommended' } = {}) {
  ...
  const seed = [
    ['welcome', { flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral, components: [createPanel({ title: 'Welcome to BlancoCountyRP', content: ABOUT_TEXT + '\n\n**Motto:** ' + MOTTO, topImage: TOP_BANNER_URL, bottomImage: BOTTOM_FOOTER_URL })] }],
    ['dashboard', { flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral, components: [buildDashboardPanel()] }],
    ['support', { flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral, components: [buildTicketPanel(), buildTicketMenu()] }],
    ['verify', { flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral, components: [buildVerificationPanel(), buildVerificationButtonRow()] }],
    ['departments', { flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral, components: [buildDepartmentsPanel()] }],
    ['applications', { flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral, components: [buildApplicationsPanel(), buildApplicationsMenu()] }],
    ['sessions', { flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral, components: [buildSessionPanel(getSessionState(guild.id), guild), buildSessionButtons(getSessionState(guild.id))] }]
  ];
  ...
  async interaction => {
    ...
    try {
      ...
      if (interaction.isChatInputCommand() && interaction.commandName === 'vote') {
        ...
        return interaction.reply({ flags: MessageFlags.IsComponentsV2 | MessageFlags.Ephemeral, components: [panel, row] });
      }
